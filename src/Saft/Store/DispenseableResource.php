...
class DispenseableResource 
{
     ...
     * @var string
     */
    protected $_uriPrefix = "http://localhost/";
    ...
    {
        return isset($this->_data[urlencode($name)]);
    }    
    ...
    public function __set($name, $value)
    {
        ...
        if ("id" == $name) {
            $this->uri = $this->_uriPrefix . urlencode($this->id) . "/";
        }
    }  
    ...
    public function load($id)
    {
        ...
        $uri = $this->_uriPrefix . urlencode($id) . "/";
        
        // get all information about a certain resource
        $resourceInformation = $this->_graph->getResourceInformation($uri);
        ...
        foreach ($resourceInformation as $entry) {
            ...
            if (false !== strpos($entry[1], ":")) {
                ...
                if (null === $property) {
                    $prefix = $this->_uriPrefix . $this->id . "/";
            ...
            $this->_data[$property] = $entry[2]["value"];
        }
    }
    ...
    {
        $this->_data = array("id" => $this->id, "uri" => $this->uri);
    }
    ...
    public function save()
    {
        ...
        // get all information about a certain resource
        $resourceInformation = $this->_graph->getResourceInformation($this->uri);
        
        // if this resource was created before remove all old resource triples
        if (null !== $resourceInformation) {
            $this->_graph->dropMultipleTriples($resourceInformation);
        }
        
        $this->_graph->addMultipleTriples($triplesToAdd);
    }    
}
