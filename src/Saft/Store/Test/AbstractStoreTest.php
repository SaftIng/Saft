...
abstract class AbstractStoreTest extends TestCase
{
    ...
    public function testAddStatementsTriples()
    {
        ...
        $statementIterator = new ArrayStatementIteratorImpl([$statement1, $statement2]);
        $query = 'INSERT DATA {
            Graph <http://graph/> {
    ...
    public function testDeleteMatchingStatementsQuadRecognition()
    {
        ...
        $this->fixture->addStatements([$statement]);
    ...
    public function testDeleteMatchingStatementsStatementsWithLiteral()
    {
        ...
        $statements = [$this->getTestTriple(), $this->getTestStatementWithLiteral()];
        $this->fixture->addStatements($statements, $this->testGraph);
    ...
    public function testGetMatchingStatements()
    {
         ...
         */
        $instanceToCheckAgainst = new ArrayStatementIteratorImpl([
            new StatementImpl(
                $this->nodeFactory->createNamedNode('http://s/'),
                $this->nodeFactory->createNamedNode('http://p/'),
                $this->nodeFactory->createNamedNode('http://o/'),
    ...
    public function testGetMatchingStatementsCheckForTriples()
    {
        ...
        // add triples
        $this->fixture->addStatements([$this->getTestTriple()], $this->testGraph);
    ...
    public function testQuery()
    {
        ...
        $setResultToCheckAgainst->setVariables(['s', 'o']);
    ...
    {
        // add test data
        $statements = [$this->getTestTriple(), $this->getTestQuad()];
        $this->fixture->addStatements($statements, $this->testGraph);

        $resultFactory = new ResultFactoryImpl();

        $this->assertResultEquals(
            $resultFactory->createStatementResult([
                new StatementImpl(
    ...
    public function testQueryEmptyResult()
    {
        ...
        $this->assertEquals(['s', 'p', 'o'], $result->getVariables());
    ...
    public function testQueryDeleteMultipleStatementsVariablePatterns()
    {
         ...
         * remove test data via query
         */
        $triplePart = $this->rdfHelpers->statementsToSparqlFormat([$this->getTestPatternStatement()]);
    ...
    public function testQueryDeleteMultipleStatementsStatementsWithLiteral()
    {
        ...
        $statements = [$this->getTestStatementWithLiteral(), $statement];
        $this->fixture->addStatements($statements, $this->testGraph);
